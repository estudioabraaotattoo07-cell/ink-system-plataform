...
export async function obterFicha360Segura(contaId: string, agora = new Date()): Promise<ResultadoFicha360> {
  ...
  const [jornada, identidade, eventos, mensagens, avaliacoes, implantacoesFortes, implantacoesLegadas, clientesFortes, clientesLegados, licencasFortes, licencasLegadas, leadsFortes, leadsLegados] = await Promise.all([
    ...
    sb.from("ink_mensagens_comerciais").select("id, conta_id, codigo, canal, status, criado_em, agendado_em, processado_em, enviado_em", { count: "exact" }).eq("conta_id", contaId).order("criado_em", { ascending: false }).limit(LIMITES_RELACIONAMENTO_360.mensagens),
    sb.from("ink_avaliacoes_comerciais").select("id, conta_id, nota, solicita_suporte, criado_em", { count: "exact" }).eq("conta_id", contaId).order("criado_em", { ascending: false }).limit(LIMITES_RELACIONAMENTO_360.avaliacoes),
  ...
  const [itensBase, historico, consumo, falhas, chamados, financeiro, authConta] = await Promise.all([
    implantacaoCandidata ? sb.from("ink_implantacao_itens").select("id, tipo, status, atualizado_em").eq("implantacao_id", implantacaoCandidata.id) : Promise.resolve({ data: [], error: null }),
    ...
    conta.auth_user_id ? sb.from("mensageria_falhas").select("id, user_id, canal, criado_em", { count: "exact" }).eq("user_id", conta.auth_user_id).order("criado_em", { ascending: false }).limit(LIMITES_RELACIONAMENTO_360.falhas) : Promise.resolve({ data: [], error: null, count: 0 }),
