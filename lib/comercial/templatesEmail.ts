...
function moldura(conteudo: string) {
  return `<!doctype html><html lang="pt-BR"><body style="margin:0;background:#0B0B0B;color:#E9E3D8;font-family:Arial,sans-serif"><div style="display:none;max-height:0;overflow:hidden">Ink System — comunicação sobre sua conta.</div><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#0B0B0B"><tr><td align="center" style="padding:28px 14px"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;background:#151515;border:1px solid #5C4B21"><tr><td style="padding:30px"><div style="font-family:Georgia,serif;font-size:28px;color:#D6B451">INK SYSTEM</div><div style="font-size:11px;letter-spacing:2px;color:#8C8376;margin-top:4px">ECOSSISTEMA PARA ESTÚDIOS</div><div style="height:1px;background:#4A402A;margin:24px 0"></div>${conteudo}<div style="height:1px;background:#332E24;margin:28px 0 20px"></div><p style="font-size:13px;line-height:1.7;color:#9C9488;margin:0">Você está entrando em um ecossistema criado por quem conhece a rotina real de um estúdio. Será uma honra acompanhar sua evolução.</p><p style="font-size:14px;color:#D6B451;margin:18px 0 0">Nos vemos dentro do Ink System.<br>Equipe Ink System</p></td></tr></table></td></tr></table></body></html>`;
}

export function montarEmailComercial({ codigo, mensagemId, nome, appUrl }: DadosTemplate) {
  ...
  return {
    ...
    html: moldura(`<div style="font-size:16px;line-height:1.75;color:#DED8CD">${mensagem.corpo}</div>`),
  };
}
