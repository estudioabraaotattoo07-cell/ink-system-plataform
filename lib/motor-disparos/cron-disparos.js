// lib/motor-disparos/cron-disparos.js — réguas de disparo automático,
// primeira responsabilidade do Motor de Disparos da Plataforma.
// Roda diariamente às 12h00 (UTC), equivalente a 09h00 em Brasília (UTC-3),
// acionado por app/api/cron-disparos/route.ts
// (Vercel Cron). Verifica réguas pós-venda, pré-venda e fluxo_etapas para
// todos os usuários. Dispara via Resend (email) ou Zenvia (WhatsApp/SMS)
// quando: prazo atingido + não enviado ainda + ativo.
//
// Portado de inq-saas/api/cron-disparos.js (Bloco 4.5). Framework-agnóstico
// de propósito: não conhece Request/Response, só devolve {status, body} —
// quem traduz isso pra HTTP é a rota. Mesmo padrão de separação já usado em
// inq-saas/lib/tenant/provisionamento/ (orquestrador não sabe o que é HTTP).
//
// Canais de envio (Resend, Zenvia) moraram aqui antes e agora vivem em
// ./canais/ — extração verbatim, sem mudança de comportamento —, pra que
// WhatsApp, notificações, campanhas e demais responsabilidades do Motor de
// Disparos possam nascer ao lado sem precisar reabrir este arquivo.

import { createClient } from "@supabase/supabase-js";
import { dispararEmail } from "./canais/email.js";
import { dispararZenvia } from "./canais/zenvia.js";

// Inicialização tardia: o cliente só é criado quando alguma função realmente
// precisa dele (não na importação do módulo), pra não derrubar o build em
// ambientes sem essas variáveis (ex.: Preview) -- ver Bloco C.
let _sb = null;
function getSupabase() {
  ...
  if (!url || !key) {
    throw new Error("Motor de Disparos: NEXT_PUBLIC_SUPABASE_URL e SUPABASE_SERVICE_KEY sao obrigatorias.");
  }
...
// aceito só como compatibilidade temporária com contas legadas. Prata/Ouro
// são legado técnico, sem versão comercial ativa -- não devem mais
// autorizar nenhum disparo automático.
// ============================================================
...
function versaoComercialDoTenant(planoLegado) {
  ...
  if (normalizado === "1.0") return "1.0";
  if (normalizado === "bronze") return "1.0"; // compatibilidade temporária
  return null;
}

// ─── D-1 LEGADO DESATIVADO ───────────────────────────────────────────────────
// O lembrete D-1 (sessão/consulta "amanhã") agora é responsabilidade EXCLUSIVA do
// inq-saas (api/cron-push.js -> api/_lib/lembreteD1.js, 09h BRT, e-mail): token por
// agenda.id (confirmacao.html#token=), motor comercial M400 (idempotência + franquia)
// e elegibilidade correta. O bloco antigo daqui gerava clientes.confirmacao_token,
// enviava /confirmar.html?token=..., escolhia o evento por cliente (.limit(1)) e
// enviava também para evento CANCELADO -- mantê-lo ligado duplicaria o novo D-1.
// Só ESTE bloco é desligado; D-0, réguas, NPS, sazonais e a fila comercial seguem.
const D1_LEGADO_ATIVO = false;

// ─── CONFIRMAÇÃO IMEDIATA LEGADA DESATIVADA ──────────────────────────────────
// O e-mail de agendamento/confirmação de Consulta/Sessão ("Sua sessão/consulta está confirmada") agora é
// responsabilidade EXCLUSIVA do inq-saas (POST /api/confirmacao?acao=enviar_agendamento -> api/_lib/emailAgendamento.js):
// autoridade = inq-saas + motor comercial M400 + agenda.id (token por agenda.id, idempotência por solicitacao_id, franquia,
// toggles e canal e-mail). O bloco antigo daqui era um SEGUNDO motor que competia com o fluxo novo:
//   - enviava direto pelo Resend do tenant, fora do M400 (sem franquia, sem idempotência por solicitacao_id);
//   - ignorava canais_habilitados e tratava qualquer canal != "sms" como e-mail (mesmo com o tenant no canal "whatsapp");
//   - escolhia "o primeiro evento do cliente" e incluía evento CANCELADO;
//   - só era contido por `__confirma_<consulta|sessao>__<etapa_desde>`, gravada pelo CRM apenas quando o e-mail novo
//     saía -- então agia como fallback de qualquer falha, bloqueio de quota, toggle/canal divergente ou série
//     recorrente (que, por regra de produto, não envia e-mail imediato por sessão);
//   - dizia "está confirmada" quando a presença ainda não foi confirmada pelo cliente.
// Falha ou bloqueio do fluxo autoritativo NÃO autoriza um segundo motor a enviar por fora.
// Só ESTE bloco é desligado (código preservado, sem apagar); o cron, D-0, réguas, NPS, sazonais e a fila comercial seguem.
// A chave `__confirma_*__` continua significando "e-mail imediato enviado de fato" (semântica do CRM, inalterada).
const CONFIRMACAO_LEGADA_ATIVA = false;
...
// ─── PROJETO ABERTO E SESSÃO CONCLUÍDA (Bloco C, 2026-08-27; termo corrigido ──
// pra "aberto" em 2026-08-28, revisão pós-auditoria com dados reais).
// Fonte de verdade para a trilha de mensagem em "aguard_agend": nunca infere
// o contexto pela etapa em si, sempre verifica o projeto e a Agenda de fato.
//
// "Projeto aberto" -- não mais "projeto ativo": a auditoria com dados reais
// encontrou clientes com projeto operacionalmente em curso marcado
// status="andamento" (grafia legada de uma fábrica de projeto mais antiga,
// ainda viva fora deste bloco -- CrmClient.tsx, fora de escopo aqui), que um
// filtro restrito a status==="ativo" deixava de reconhecer, classificando
// clientes com sessão já concluída como se fossem pré-1ª-sessão. A convenção
// já dominante e validada no resto do sistema (dezenas de ocorrências em
// CRM.tsx e CrmClient.tsx) nunca enumera os valores de "aberto" -- exclui só
// os dois valores terminais conhecidos: "concluido" e "cancelado". Este
// helper segue a mesma convenção por exclusão, de propósito, em vez de
// manter uma lista fechada mais restritiva do que o resto do sistema.
//
// Só é considerado quando exatamente 1 projeto está aberto -- 0 ou mais de
// 1 é ambíguo e NUNCA resolvido por heurística (retorna null; o chamador
// cai para a trilha pré-1ª-sessão por segurança, nunca assume "entre
// sessões" sem prova).

function identificarProjetoAberto(projetos) {
  const abertos = (projetos || []).filter((p) => p && p.status !== "concluido" && p.status !== "cancelado");
  return abertos.length === 1 ? abertos[0] : null;
}

// Prova de que já houve ao menos uma sessão (ou piercing) concluída
// especificamente do projeto aberto -- casamento exato de projeto_id, sem
// fallback para eventos com projeto_id nulo (esses não contam nem a favor
// nem contra).
async function temSessaoConcluidaDoProjetoAtivo(clienteId, projetoAtivoId) {
  if (!projetoAtivoId) return false;
  const sb = getSupabase();
  try {
    const { data } = await sb
      .from("agenda")
    ...
    return (data || []).some(
      (e) =>
        (e.tipo === "piercing" || (e.tipo || "").startsWith("sess")) &&
        e.projeto_id != null &&
        String(e.projeto_id) === String(projetoAtivoId)
    );
  } catch {
    return false;
  }
...
// Sem req/res de propósito: quem autentica a chamada (segredo do Cron) e
// traduz o retorno em resposta HTTP é app/api/cron-disparos/route.ts. Este
// módulo só sabe executar a régua e devolver {status, body}.
...
export async function executarMotorDisparos() {
  ...
  try {
    ...
    if (cfgErr || !configs) {
      return { status: 500, body: { error: "Erro ao buscar configuracoes" } };
    }
    ...
    for (const cfg of configs) {
      ...
      try {
        const { data: cliData } = await sb
          .from("clientes")
          .select("id, nome, email, tel, etapa, etapa_desde, nascimento, sessao_concluida_em, disparos_enviados, hist, followups, confirmacao_token, confirmacao_token_exp, confirmacao_evento_id, confirmacao_presenca, artista, descricao, avaliacao_fluxo_status, avaliacao_token, avaliacao_token_exp, google_convite_em, projetos")
          .eq("user_id", userId)
          .is("excluido_em", null);
      ...
      for (const cliente of clientes) {
        ...
        // ── AGUARDANDO AGENDAMENTO → HIBERNAÇÃO — D+60 (Bloco C, 2026-08-27) ────
        // Consolida em uma única regra de tempo o que antes eram dois relógios
        // (D+90 a partir de aguard_prox_sessao e nenhum limite automático a
        // partir de aguard_1a_sessao): decisão de produto fechada é que ambas
        // as trilhas (pré-1ª-sessão e entre-sessões) hibernam em D+60 a partir
        // da entrada em aguard_agend, sem mensagem própria neste ponto -- só a
        // mensagem D+30 (pré-1ª-sessão) ou D+2 (entre-sessões), mais abaixo,
        // tentam reengajar antes disso. Mesmo padrão de concorrência do Bloco D
        // (pos_venda->reengajamento): UPDATE condicional no banco por
        // id+user_id+etapa+etapa_desde, .select("id"), histórico só se o
        // UPDATE realmente afetou uma linha. totalDisparos NÃO é incrementado
        // -- é um Pipeline move, não um disparo de mensagem.
        if (cliente.etapa === "aguard_agend" && cliente.etapa_desde) {
          const diasEmAguardAgend = diasEntre(cliente.etapa_desde, hoje);
          if (diasEmAguardAgend >= 60) {
            const { data: linhasAfetadasAguardAgend } = await sb.from("clientes")
              .update({ etapa: "hibernacao", etapa_desde: new Date().toISOString() })
              .eq("id", cliente.id)
              .eq("user_id", userId)
              .eq("etapa", "aguard_agend")
              .eq("etapa_desde", cliente.etapa_desde)
              .select("id");
            if (linhasAfetadasAguardAgend && linhasAfetadasAguardAgend.length > 0) {
              await registrarHistorico(userId, "Movido automaticamente para Hibernação após 60 dias em Aguardando Agendamento — " + cliente.nome);
            }
          }
        }

        // ── PÓS-VENDA → REENGAJAMENTO — D+7, sem depender de resposta de NPS ────
        // Bloco D (revisão de concorrência, 2026-08-27). `cliente` foi lido no
        // início deste ciclo do cron -- entre essa leitura e este UPDATE, o
        // operador pode ter tirado o cliente de pos_venda de verdade (novo
        // agendamento real, via AUTO_MOVE_ORIGENS). Um UPDATE só por
        // .eq("id", ...) sobrescreveria esse estado novo com "reengajamento"
        // por engano -- por isso o WHERE também exige etapa E etapa_desde
        // ainda serem exatamente os lidos (proteção dupla: cobre tanto uma
        // troca de etapa quanto uma saída-e-reentrada em pos_venda com um
        // etapa_desde mais novo). .select("id") devolve as linhas realmente
        // afetadas -- histórico só é gravado se o UPDATE de fato aconteceu,
        // nunca em no-op. Sem chave de dedup artificial -- a condição de
        // etapa (agora also validada no próprio UPDATE) já é a trava.
        // totalDisparos NÃO é incrementado aqui: o campo representa disparos
        // de mensagem (é devolvido como "disparos" na resposta do cron,
        // linha ~1347), e mover uma etapa de Pipeline não envia nada. O
        // precedente que incrementava indevidamente num Pipeline move
        // (aguard_prox_sessao->hibernacao) foi removido pelo Bloco C -- a
        // nova Hibernação D+60 (aguard_agend->hibernacao, acima) já segue
        // este mesmo racional corrigido desde a origem.
        if (cliente.etapa === "pos_venda" && cliente.etapa_desde) {
          const diasEmPosVenda = diasEntre(cliente.etapa_desde, hoje);
          if (diasEmPosVenda >= 7) {
            const { data: linhasAfetadasPosVenda } = await sb.from("clientes")
              .update({ etapa: "reengajamento", etapa_desde: new Date().toISOString() })
              .eq("id", cliente.id)
              .eq("user_id", userId)
              .eq("etapa", "pos_venda")
              .eq("etapa_desde", cliente.etapa_desde)
              .select("id");
            if (linhasAfetadasPosVenda && linhasAfetadasPosVenda.length > 0) {
              await registrarHistorico(userId, "Movido automaticamente de Pós-venda para Reengajamento após 7 dias — " + cliente.nome);
            }
          }
        }
...
<p style="font-size:12px;color:#bbb;margin-top:24px">${studioName}</p>
        ...
        // ── AGUARDANDO AGENDAMENTO — D+0 boas-vindas + D+30 recontato, trilha ──
        // pré-1ª-sessão (Bloco C, revisão 2026-08-27 pós-auditoria). Fundiu as
        // antigas etapas "Aguardando 1ª Sessão" (aguard_1a_sessao) e
        // "Aguardando Nova Solicitação de Projeto" (aguard_prox_sessao) na
        // etapa operacional "aguard_agend". A distinção de contexto não é
        // mais inferida pela etapa em si -- é resolvida por projeto, via
        // Agenda (fonte de verdade), nunca por heurística ou fallback:
        // projeto ativo ambíguo (0 ou >1 com status "ativo") ou sem sessão
        // comprovada do projeto ativo cai na trilha pré-1ª-sessão por
        // segurança.
        //
        // Decisão de produto reconfirmada após a primeira auditoria pós: o
        // e-mail de boas-vindas D+0 (antigo agradecimento_1asessao) FOI
        // trazido de volta para a régua consolidada -- a retirada anterior
        // não era uma decisão de produto autorizada. Régua final: D+0
        // (boas-vindas, imediato) + D+30 (recontato único, sem repetição) +
        // D+60 (Hibernação, bloco acima). Ambos D+0 e D+30 usam o mesmo
        // contexto de projeto ativo (aguardAgendTemSessaoConcluida) e
        // reaproveitam exatamente as mesmas chaves de dedup, textos e campos
        // de toggle da antiga aguard_1a_sessao (bv / d30) -- clientes que já
        // as receberam sob a etapa antiga não as recebem de novo.
        let aguardAgendTemSessaoConcluida = false;
        if (cliente.etapa === "aguard_agend" && cliente.etapa_desde) {
          const projetoAbertoAguardAgend = identificarProjetoAberto(cliente.projetos);
          aguardAgendTemSessaoConcluida = projetoAbertoAguardAgend
            ? await temSessaoConcluidaDoProjetoAtivo(cliente.id, projetoAbertoAguardAgend.id)
            : false;

          if (!aguardAgendTemSessaoConcluida && cfg.resend_api_key && cliente.email) {
            const fn = (cliente.nome || "").trim().split(" ")[0];
            let nomeArtista = "";
            if (cliente.artista) {
              try {
            ...
            // D+0: e-mail imediato de boas-vindas/orientação (mesmo texto e
            // chave de dedup da antiga aguard_1a_sessao_bv)
            const jaEnviouBV = disparosEnviados && disparosEnviados["__aguard_1a_sessao_bv__"];
            if (cfg.fluxo_agradecimento_1asessao_ativa !== false && !jaEnviouBV) {
              const diasEtapaBV = diasEntre(cliente.etapa_desde, hoje);
              if (diasEtapaBV >= 0) {
...
<p style="font-size:12px;color:#bbb;margin-top:24px">Respeitoso abraço, ${studioName}</p>
                ...
                const okBV = await dispararEmail({
                  apiKey: cfg.resend_api_key,
                  from: cfg.email_remetente,
                  nome_remetente: studioName,
                  to: cliente.email,
                ...
                if (okBV) {
                  let disparosAtuaisBV = {};
                  try { const { data: cliAtual } = await sb.from("clientes").select("disparos_enviados").eq("id", cliente.id).single(); disparosAtuaisBV = cliAtual?.disparos_enviados || {}; } catch {}
                  await marcarEnviado(cliente.id, "__aguard_1a_sessao_bv__", disparosAtuaisBV);
                  await registrarHistorico(userId, "E-mail boas-vindas Aguardando Agendamento (pré-1ª sessão) enviado — " + cliente.nome);
                  totalDisparos++;
                }
              }
            }

            // D+30: recontato único, sem repetição (D+60 hiberna, bloco
            // acima, em vez de reenviar)
            const jaEnviouD30 = disparosEnviados && disparosEnviados["__aguard_1a_sessao_d30__"];
            if (cfg.fluxo_recontato_d30_ativa !== false && !jaEnviouD30) {
              const diasEtapa = diasEntre(cliente.etapa_desde, hoje);
              if (diasEtapa >= 30) {
...
<p style="font-size:11px;color:#bbb;text-align:center">Sem problemas — seu projeto continua guardado com a gente até você estar pronto(a).</p>
<p style="font-size:12px;color:#bbb;margin-top:24px">Respeitoso abraço, ${studioName}</p>
</div>`;
                const ok = await dispararEmail({
                  apiKey: cfg.resend_api_key,
                ...
                if (ok) {
                  ...
                  await registrarHistorico(userId, "E-mail recontato D+30 Aguardando Agendamento (pré-1ª sessão) enviado — " + cliente.nome);
                  totalDisparos++;
                }
              }
            }
        ...
        // DESATIVADO (CONFIRMACAO_LEGADA_ATIVA = false, ver o comentário da constante): a autoridade é o inq-saas + M400.
        if (CONFIRMACAO_LEGADA_ATIVA && (cliente.etapa === "sessao_agend" || cliente.etapa === "cons_agendada") && cliente.etapa_desde && ((cfg.resend_api_key && cliente.email) || (cfg.zenvia_api_key && cliente.tel))) {
        ...
        if (D1_LEGADO_ATIVO && temDisparosPrata && cfg.fluxo_confirmacao_presenca_ativa !== false && (cliente.etapa === "sessao_agend" || cliente.etapa === "cons_agendada")) {
          const ehConsulta = cliente.etapa === "cons_agendada";
          try {
        ...
        if (temDisparosPrata) {
        // Bloco C (2026-08-27): só dispara a régua "entre sessões" quando há
        // prova de sessão concluída do projeto ativo (aguardAgendTemSessaoConcluida,
        // calculado acima) -- sem essa prova, a trilha correta é a
        // pré-1ª-sessão (D+30, bloco acima), nunca esta.
        if (cliente.etapa === "aguard_agend" && aguardAgendTemSessaoConcluida) {
    ...
    return {
      status: 200,
      body: { ok: true, disparos: totalDisparos, erros: totalErros, executado_em: hoje.toISOString() }
    };
  } catch (err) {
    return { status: 500, body: { error: "Erro interno", detail: String(err.message || err) } };
  }
}
