...
(function(){
  ...
  // Bloco de Idempotência de Interação Pública (2026-09-02): uma chave por
  // interação (não por PII, não por cliente) -- gerada no navegador antes
  // da primeira submissão, reutilizada em toda chamada subsequente da
  // MESMA conversa, inclusive depois de um reload/retry na mesma aba
  // (sessionStorage, nunca localStorage -- não deve sobreviver o
  // fechamento da aba/navegador). Não é autorização, não é segredo, não
  // contém PII -- é só um UUID aleatório. Não há hoje nenhuma ação de
  // "reiniciar conversa" nesta interface (o widget é aberto/fechado, nunca
  // reiniciado) -- se uma vier a existir, ela precisa gerar uma chave nova
  // nesse ponto; até lá, uma única chave por aba cobre o ciclo de vida
  // real do widget.
  var CHAVE_IDEMPOTENCIA_STORAGE_KEY = 'aura_chave_idempotencia';
  function obterChaveIdempotencia(){
    try {
      var existente = sessionStorage.getItem(CHAVE_IDEMPOTENCIA_STORAGE_KEY);
      if (existente) return existente;
      var nova = crypto.randomUUID();
      sessionStorage.setItem(CHAVE_IDEMPOTENCIA_STORAGE_KEY, nova);
      return nova;
    } catch (e) {
      // sessionStorage indisponível (ex.: navegação privada restrita) --
      // fail-open pra uma chave só-desta-chamada: perde a proteção de
      // idempotência entre reloads, mas nunca quebra o envio em si.
  ...
  function salvar(campos){
    ...
    delete payload._credencial;
    delete payload._credencialUpload;
    if (lead._clienteId) payload.clienteId = lead._clienteId;
    // Bloco de Continuidade Autorizada (2026-09-01): clienteId sozinho não é
    // mais suficiente pra continuar a mesma conversa -- precisa vir junto
    // com a credencial assinada que /api/lead devolveu na resposta anterior
    // (token opaco, nunca inspecionado aqui, só guardado e reenviado). Sem
    // ela, o servidor ainda reconhece a pessoa por nome/telefone/e-mail
    // (dedup por PII), mas trata qualquer conteúdo novo como uma nova
    // solicitação em vez de continuar editando a mesma.
    if (lead._credencial) payload.credencial = lead._credencial;
    // Enviada em TODA chamada, inclusive a primeira -- é a primeira
    // submissão (ou seu retry) que mais precisa da proteção contra
    // duplicação, antes de existir qualquer clienteId/credencial.
    payload.chave_idempotencia = obterChaveIdempotencia();
    ...
    return fetch(API_BASE + '/api/lead', {
      ...
      .then(function(data){
        ...
        if (data && data.credencial) lead._credencial = data.credencial;
        // credencialUpload é uma credencial SEPARADA (purpose=
        // referencia_upload) -- nunca a mesma de lead._credencial
        // (purpose=lead_continuidade). api/upload.js rejeita a errada por
        // domain separation, então guardamos as duas.
        if (data && data.credencialUpload) lead._credencialUpload = data.credencialUpload;
  ...
  function comprimirEEnviar(file, cb){
    ...
    reader.onload = function(ev){
      ...
      img.onload = function(){
        ...
        // Bloco de Continuidade Autorizada (2026-09-01): api/upload.js
        // agora exige uma credencial assinada com purpose=referencia_upload
        // junto com clienteId pra vincular a referência -- sem ela, o
        // arquivo ainda é salvo, mas fica sem dono. É uma credencial
        // DIFERENTE da usada em /api/lead (lead._credencial, purpose=
        // lead_continuidade) -- domain separation faz a de lá nunca validar
        // aqui, então usamos lead._credencialUpload, guardada pelo mesmo
        // passo salvar() acima a partir de data.credencialUpload.
        fetch(API_BASE + '/api/upload', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ base64: base64, mimeType: 'image/jpeg', clienteId: lead._clienteId, credencial: lead._credencialUpload })
